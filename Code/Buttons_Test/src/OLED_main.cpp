...
#define MAX_TXT_ROWS 6 //Test viewer can fit 6 rows (no header)
...
void readAndDisplayTxt(String filePath) {
  ...
  if (!file) {
    ...
    display.clearDisplay();
    display.print("Failed to open text file");
    display.display();
    return;
  }
  ...
  while (file.available()) {
    ...
    line.trim(); //Remove trailing carriages or whitespaces
  ...
  //Local loop dedicated to viewing the text file contents
  while (insideViewer) {
    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(WHITE);

    //Render the slice of text lines visible on screen
    for (int i = 0; i < MAX_TXT_ROWS; i++) {
      int lineIdx = txtScrollIdx + i;
      if (lineIdx >= (int)fileLines.size()) break;

      display.setCursor(0, i * 10); // 10 pixel spacing per line
      display.print(truncateName(fileLines[lineIdx], SCREEN_WIDTH));
    }
    display.display();

    // Handle navigation inside the file viewer
    ...
    //Press LEFT to exit the text file viewer
    if (isButtonPressed(Left)) {
      insideViewer = false;
    }

    //Keep volume buttons fonctional while reading a file
        ...
        if (!currentFiles.empty()) {
            ...
            if (selected.isDir) {
                ...
                if (lowerName.endsWith(".txt")) {
                  readAndDisplayTxt(fullPath);
                  stateChanged = true; //Refresh the explorer layout on return
                }
                else {
                  Serial.print("Unsupported file: ");
