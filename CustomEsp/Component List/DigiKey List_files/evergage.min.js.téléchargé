...
const evg_OTCookieName = 'OptanonConsent';
const evg_OTCookieMatch = document.cookie.match(new RegExp('(^| )' + evg_OTCookieName + '=([^;]+)'));
if(evg_OTCookieMatch) {
    const evg_OTCookieValue = evg_OTCookieMatch[2];
    if (evg_OTCookieValue && evg_OTCookieValue.includes('C0002')) {
        // user has opted-in
    } else {
        // user has opted-out
        try {
...
Evergage.currentLanguage = getValueFromWindow("window.__headerData.lang");
